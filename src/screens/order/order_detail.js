...
export class Order extends XComponent {
    ...
    init() {
        try {
            this.dic = {
                ...
                classSelectedSymbol: this.getClassBySymbol({
    ...
    changeBuySell(isBuy) {
        try {
            ...
            logDevice(
                'info',
                `NewOrder - changeBuySell - ${tradeType}: ${
    ...
    onChangeText(type, val) {
        try {
            ...
            logDevice(
                'info',
                `NewOrder - onChangeText ${type} - ${val}: ${
                    error ? JSON.stringify(error) : ''
                }`
            );
        }
 ...
 return false;
    ...
    async setCode(symbol, company, classSymbol, isInit, isRefresh) {
        try {
            this.dic.classSelectedSymbol = this.getClassBySymbol({
                symbol,
                classSymbol
            });
        ...
        return (
            <TouchableWithoutFeedback onPress={this.handleDismissKeyboard}>
                ...
                >
                    ...
                    {content}
                    {/* {this.renderSummary()} */}
                    <View style={{ flex: 1 }}>{this.renderTabScroll()}</View>
    ...
    render() {
        ...
        return (
            <NestedScrollView
                ...
                renderHeaderPanner={this.renderHeaderPanel}
            >
                <View
                    style={{
                        position: 'absolute',
