...
let userId = null;
var perf = null;
export function loadNewsData(newType, filterTypeNew = Enum.FILTER_TYPE_NEWS.ALL, pageID, pageSize, isLoadMore, loading = false, isChangeTab) {
    return dispatch => {
        try {
            ...
            getData(dispatch, newType, filterTypeNew, pageID, pageSize, isLoadMore, isChangeTab);
        } catch (error) {
...
export function getData(dispatch, newType, filterTypeNew, pageID, pageSize, isLoadMore, isChangeTab) {
    const isOnWatchlist = newType === Enum.TYPE_NEWS.RELATED
    if (isOnWatchlist) {
        getRelatedNews(dispatch, newType, filterTypeNew, pageID, pageSize, isLoadMore, isChangeTab)
    } else {
        getEverythingNews(dispatch, newType, filterTypeNew, pageID, pageSize, isLoadMore, isChangeTab)
    }
}
...
function getStringQuerryFromDataStorage() {
    let stringQuery = '';
...
function getRelatedSymbolFromDic(isLoadMore, cb, isChangeTab) {
    if (!isLoadMore && !isChangeTab) {
        analysSymbol()
            .then(() => {
                ...
                cb && cb(stringQuery, isLoadMore, isChangeTab);
            })
            .catch(error => {
                console.log('error at getRelatedSymbolfromDic', error)
            });
    } else {
        const stringQuery = getStringQuerryFromDataStorage()
...
function getRelatedNews(dispatch, newType, filterTypeNew, pageID, pageSize, isLoadMore, isChangeTab) {
    let urlNewsInQuery = '';
    const cb = (stringQuery, isLoadMore, isChangeTab) => {
        if (!stringQuery) {
        ...
        urlNewsInQuery = `${urlNewsInQuery}&duration=year`
    ...
    getRelatedSymbolFromDic(isLoadMore, cb, isChangeTab)
}
...
function getEverythingNews(dispatch, newType, filterTypeNew, pageID, pageSize, isLoadMore, isChangeTab) {
    let url = '';
    const stringQuery = ''
    // News truyền thêm from to còn không thì lấy news trong ngày -> to = thời điểm hiện tại - from = lùi về 0:00 7h ngày trước VD: from = 0h00 1/8 -> 7/8
    url = api.getNewsUrl(newType, filterTypeNew, stringQuery, pageID, pageSize);
