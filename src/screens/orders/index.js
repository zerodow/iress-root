...
import {
  useShowHideTabbar,
  useShowSearchAccount, useSetDetailSpaceTop
...
const OrdersWrapper = ({ navigator }) => {
  ...
  const [refDetail, showDetail, hideDetail, updateDataRealtime] = useShowDetail()
    ...
    switch (event.id) {
      ...
      case 'didDisappear':
        if (dataStorage.tabIndexSelected !== 4) {
          dic.current.timeoutLoadingPanel && clearTimeout(dic.current.timeoutLoadingPanel)
  ...
  return isFirstLoad
    ? <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: CommonStyle.backgroundColor1 }}>
