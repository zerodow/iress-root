    ...
    return <View style={{
        ...
        <Text style={{
            ...
            <Text style={{
                fontFamily: CommonStyle.fontPoppinsRegular,
                fontSize: CommonStyle.font11,
                color: filledQuantity === orderQuantity
            ...
            <Text style={{
                ...
                {` / ${formatNumberNew2(orderQuantity, PRICE_DECIMAL.VOLUME)}`}
            </Text>
        </Text>
    </View>
}
...
const Chart = ({ side, filledQuantity = 0, orderQuantity }) => {
    ...
    useEffect(() => {
        if (filledQuantity === null || filledQuantity === undefined || !orderQuantity) return
    ...
    const progressColor = (side + '').toUpperCase() === SIDE.BUY
        ? CommonStyle.color.buy
        : CommonStyle.color.sell
...
const FilledPriceAndPercent = ({ side, filledQuantity, orderQuantity, filledPrice }) => {
    const volumePercent = filledQuantity / orderQuantity * 100
    const color = (side + '').toUpperCase() === SIDE.BUY ? CommonStyle.color.buy : CommonStyle.color.sell
    const keyLanguage = filledQuantity === orderQuantity ? 'filledPercentDesc' : 'unfilledPercentDesc'
    return <View style={{ width: '100%', alignItems: 'center', marginTop: 4 }}>
        <Text style={{ flexDirection: 'row' }}>
            <Text style={{
                ...
                {`${formatNumberNew2(volumePercent, PRICE_DECIMAL.PERCENT)}% `}
            </Text>
            <Text style={{
                fontFamily: CommonStyle.fontPoppinsRegular,
                fontSize: CommonStyle.fontTiny,
