...
import { getClassBySymbolAndExchange, getCompanyName, getClassTagProperty } from '~/business'
...
const Side = ({ side }) => {
    const backgroundColor = side === SIDE.BUY ? CommonStyle.color.buy : CommonStyle.color.sell
...
const Company = ({ symbol, exchange }) => {
    ...
    const company = getCompanyName({ symbol: singleSymbol, exchange }) || 'BHP Biliton Companry'
    return <Text
        numberOfLines={1}
        style={[
            {
                ...
                fontFamily: CommonStyle.fontPoppinsRegular,
                paddingRight: 8
            }, Platform.OS === 'android' ? { lineHeight: CommonStyle.font11 + 4 } : {}
        ]}>
        {company}
    </Text>
}
...
const SymbolClass = ({ symbol, exchange }) => {
    if (!symbol || !exchange) return null
    const singleSymbol = symbol.split('.')[0]
    const classSymbol = getClassBySymbolAndExchange({ symbol: singleSymbol, exchange }) || 'equity'
...
const SymbolInfo = ({ symbol, exchange }) => {
    return <View style={{ marginLeft: 62 + 8, justifyContent: 'space-around', flex: 1, alignItems: 'flex-end' }}>
...
const OrdersDetailSymbolInfo = ({ data }) => {
    ...
    return <View>
        ...
        <View
            onLayout={onLayout}
            style={{ zIndex: 10, paddingVertical: 8, backgroundColor: CommonStyle.color.dark }}>
            <View style={{
                marginHorizontal: 16,
                height: 62
            }}>
