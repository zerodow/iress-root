...
import { getOrderStatusProperty, getTextInsideCircleProperty } from '~s/orders/Controller/OrdersStatusController'
const OrdersDetail = ({ data, navigator }) => {
    ...
    const {
        backgroundColor
    } = getOrderStatusProperty({ orderAction, actionStatus })
    ...
    return <View style={{ width: '100%', backgroundColor: CommonStyle.color.dark }}>
        <NetworkWarning backgroundColorBottom={backgroundColor} navigator={navigator} />
        <OrdersStatus
        ...
        <OrdersDetailHistory navigator={navigator} data={data} textInsideCircle={text} />
        {
