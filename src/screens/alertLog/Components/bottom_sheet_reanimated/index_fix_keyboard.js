...
const { lessOrEq, call, set, cond, onChange, block, eq, greaterOrEq, not, defined, max, add, and, Value, spring, or, divide, greaterThan, sub, event, diff, multiply, clockRunning, startClock, stopClock, decay, Clock, lessThan, neq } = Animated;
    ...
    static getDerivedStateFromProps(props, state) {
        ...
        if (state && state.snapPoints) {
            ...
            snapPoints = sortedPropsSnapPints.map((p, i) => {
                if (i === 0) {
                    return props.top ? new Value(props.top) : new Value(sortedPropsSnapPints[0].val - p.val)
                } else {
                ...
                <Animated.View style={{
                    ...
                    <KeyboardAvoidingView
                        ...
                        >
                            ...
                            {(this.props.onOpenEnd || this.props.onCloseStart) && (
                                <Animated.Code
                                    exec={onChange(this.translateMaster, [
                                        cond(
                                            and(
                                                eq(this.translateMaster, this.props.top || this.state.snapPoints[this.state.snapPoints.length - 1]),
                            ...
                            <HandleDismissKeyboard parentState={this.panMasterState} childrenState={this.panState} />
                        </View>
                    </KeyboardAvoidingView>
                </Animated.View>
            </React.Fragment >
