...
import { isShowPrice, isShowTradingPrice, getValueToPrice, getValueToStopPrice, getValueToTakeProfitLossPrice, getValueToVolume } from '~/screens/confirm_order/Controllers/ConfirmAmendOrderController.js'
...
const RowOutstandingVolume = ({ volumeFrom, volumeTo }) => {
    return (
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', width: '100%', paddingTop: 8 }}>
            ...
            <View style={{ marginLeft: 16, width: width - 16 * 2 - 16 * 2 - widthToFrom * 2 - 16, paddingRight: 8 }}>
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'flex-end', marginRight: 16 }}>
                <View style={{ width: widthToFrom, alignItems: 'flex-end' }}>
                    <Text style={{
                        ...
                        fontFamily: CommonStyle.fontPoppinsRegular
                    }}>{formatNumberNew2(volumeFrom, Enum.PRICE_DECIMAL.VOLUME)}</Text>
                </View>
                <View style={{ width: widthToFrom, alignItems: 'flex-end', marginLeft: 16 }}>
                    <Text style={{
                        color: CommonStyle.color.modify,
                        fontSize: CommonStyle.font11,
                        fontFamily: CommonStyle.fontPoppinsRegular
                    }}>{formatNumberNew2(volumeTo, Enum.PRICE_DECIMAL.VOLUME)}</Text>
                </View>
            </View>
        </View>
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'flex-end', marginRight: 16 }}>
                <View style={{ width: widthToFrom, alignItems: 'flex-end' }}>
                    <Text style={{
                        ...
                        fontSize: CommonStyle.font11,
                        fontFamily: CommonStyle.fontPoppinsRegular
                    }}>{formatNumberNew2(priceFrom / divide, 2)}</Text>
                </View>
                <View style={{ width: widthToFrom, alignItems: 'flex-end', marginLeft: 16 }}>
                    <Text style={{
                        color: CommonStyle.color.modify,
                        fontSize: CommonStyle.font11,
                        fontFamily: CommonStyle.fontPoppinsRegular
                    }}>{formatNumberNew2(priceTo / divide, 2)}</Text>
                </View>
            </View>
        </View>
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', width: '100%', paddingTop: 8 }}>
            <View style={{ marginLeft: 16, width: width - 16 * 2 - 16 * 2 - widthToFrom * 2 - 16, paddingRight: 8 }}>
                <Text numberOfLines={1} style={{
                    ...
                    opacity: 0.5,
                    fontFamily: CommonStyle.fontPoppinsRegular
                }}>{I18n.t('stop_price') + ` (${fromCurrency})`} </Text>
            </View>
            <View style={{ flexDirection: 'row', justifyContent: 'flex-end', marginRight: 16 }}>
                <View style={{ width: widthToFrom, alignItems: 'flex-end' }}>
                    <Text style={{
                        ...
                        fontSize: CommonStyle.font11,
                        fontFamily: CommonStyle.fontPoppinsRegular
                    }}>{formatNumberNew2(stopPriceFrom ? stopPriceFrom / divide : stopPriceFrom, 2)}</Text>
                </View>
                <View style={{ width: widthToFrom, alignItems: 'flex-end', marginLeft: 16 }}>
                    <Text style={{
                        color: CommonStyle.color.modify,
                        fontSize: CommonStyle.font11,
                        fontFamily: CommonStyle.fontPoppinsRegular
                    }}>{formatNumberNew2(stopPriceTo ? stopPriceTo / divide : stopPriceTo, 2)}</Text>
                </View>
            </View>
        </View>
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', width: '100%', paddingVertical: 8 }}>
            <View style={{ marginLeft: 16, width: width - 16 * 2 - 16 * 2 - widthToFrom * 2 - 16, paddingRight: 8 }}>
                <Text numberOfLines={1} style={{
                    ...
                    opacity: 0.5,
                    fontFamily: CommonStyle.fontPoppinsRegular
                }}>{I18n.t('take_profit_price') + ` (${fromCurrency})`}</Text>
            </View>
            <View style={{ flexDirection: 'row', justifyContent: 'flex-end', marginRight: 16 }}>
                <View style={{ width: widthToFrom, alignItems: 'flex-end' }}>
                    <Text style={{
                        ...
                        fontSize: CommonStyle.font11,
                        fontFamily: CommonStyle.fontPoppinsRegular
                    }}>{formatNumberNew2(takeProfitFrom ? takeProfitFrom / divide : takeProfitFrom, 2)}</Text>
                </View>
                <View style={{ width: widthToFrom, alignItems: 'flex-end', marginLeft: 16 }}>
                    <Text style={{
                        color: CommonStyle.color.modify,
                        fontSize: CommonStyle.font11,
                        fontFamily: CommonStyle.fontPoppinsRegular
                    }}>{formatNumberNew2(takeProfitTo ? takeProfitTo / divide : takeProfitTo, 2)}</Text>
                </View>
            </View>
        </View>
...
const RowOutstanding = React.memo(({ newOrder, orderType, isBuy, limitPrice, data, symbol, exchange }) => {
    const fromCurrency = getFromCurrency()
    ...
    const { takeprofit_trigger_price: takeProfitFrom } = data.takeprofit_order_info || {}
    const { stoploss_trigger_price: stopPriceFrom } = data.stoploss_order_info || {}
    const { limit_price: priceFrom, remaining_quantity: volumeFrom } = data
    const [ShadowRowInfoPrice, onLayout] = useShadow()
    return (
        <View onLayout={onLayout}>
            <ButtonBySell isBuy={isBuy} value={symbolClass} symbol={displayName} companyName={companyName} />
            <ShadowRowInfoPrice />
            <View style={{ paddingBottom: 8 }}>
                ...
                <RowOutstandingVolume volumeFrom={volumeFrom} volumeTo={getValueToVolume(volumeFrom, volumeTo)} />
                {isShowPrice(orderType) ? <RowPrice toCurrency={fromCurrency} priceFrom={priceFrom} priceTo={getValueToPrice(priceFrom, limitPrice)} /> : null}
                {
                    isShowTradingPrice({ stopPrice: stopPriceTo, takeProfitLoss: takeProfitTo }) && ([
                        <RowStopPrice toCurrency={fromCurrency} stopPriceFrom={stopPriceFrom} stopPriceTo={getValueToStopPrice(stopPriceFrom, stopPriceTo)} />,
                        <RowTakeProfit toCurrency={fromCurrency} takeProfitFrom={takeProfitFrom} takeProfitTo={getValueToTakeProfitLossPrice(takeProfitFrom, takeProfitTo)} />
                    ])
                }
            </View>
        </View>
