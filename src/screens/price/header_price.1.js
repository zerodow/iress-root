...
class HeaderPrice extends XComponent {
	static propTypes = {
	...
	subHalt() {
		try {
			...
			Emitter.addListener(channel, this.id, () => {
				...
				FuncUtil.checkTradingHalt(this.props.symbol).then(snap => {
					const tradingHalt =
						snap && snap.trading_halt ? snap.trading_halt : false;
	...
	renderLeftTop(data, displayName) {
		const isNewsToday = this.props.newsToday[data.symbol] || false;
		try {
	...
	formatFuncTradePrice(value = {}) {
		try {
			return FuncUtil.formatNumberNew2(value.trade_price, 3);
		} catch (error) {
			console.catch(error);
			return null;
		}
		...
		try {
			return (
				<HighLightText
					...
					base={FuncUtil.formatNumberNew2(value.change_percent, 2)}
					...
					value={
						isLoading
							...
							: value.change_percent != null
								? FuncUtil.formatNumberNew2(value.change_percent, 2)
								: null
					}
				/>
			);
		...
		try {
			return (
				<HighLightText
					...
					addSymbol
					base={FuncUtil.formatNumberNew2(value.change_point, 3)}
					testID={`${value.symbol}changePoiWL`}
					value={
						isLoading
							? '--'
							: value.change_point != null
								? FuncUtil.formatNumberNew2(value.change_point, 3)
								: null
					}
				/>
			);
	...
	render() {
		try {
			...
			const companyName = func
				.getCompanyName(this.props.symbol)
				.toUpperCase();
					...
					<View style={{ flexDirection: 'row' }}>
						...
						{isTopValuePriceboard ? (
							<PricePieces
								...
								channelLv1FromComponent={
									this.props.channelLv1FromComponent
								}
								isLoading={this.props.isLoading}
								channelLoadingTrade={
									this.props.channelLoadingTrade
								}
...
const mapStateToProps = state => ({
	newsToday: state.trade.newsToday
});
