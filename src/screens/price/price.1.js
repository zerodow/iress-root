...
export class Price extends XComponent {
                                ...
                                : <View>
                                    <View style={{ width: '100%' }}>
                                        <TouchableOpacity
                                            ...
                                            disabled={!RoleUser.checkRoleByKey(Enum.ROLE_DETAIL.PERFORM_MORE_BUTTON)}
                                            style={[styles.rowExpandNews, { width: '100%' }]}>
                                            <Text
                                                testID={`more`}
                                                style={[CommonStyle.textMain, { color: RoleUser.checkRoleByKey(Enum.ROLE_DETAIL.PERFORM_MORE_BUTTON) ? '#10a8b2' : '#bbb' }]}>
                                    ...
                                    </View>
                                    {
                                        Controller.getLoginStatus()
                                            ? (
                                                RoleUser.checkRoleByKey(Enum.ROLE_DETAIL.VIEW_NEWS_OF_SYMBOL) ? <Announcement
                                                        symbol={this.props.symbol}
                                                        setting={this.props.setting}
                                                        isConnected={this.props.isConnected}
                                                        navigator={this.props.navigator}
                                                        channelReload={this.props.channelLoadingTrade} /> : (
                                                            <View style={{ height: 50, paddingHorizontal: 16, justifyContent: 'center', alignItems: 'center' }}>
                                                                {
    ...
    onChangeAllowRender(changedRows) {
        try {
            if (!changedRows || changedRows[this.props.rowID] == null) return false
