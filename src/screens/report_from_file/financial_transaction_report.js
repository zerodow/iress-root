...
export default class FinancialTransactionsReport extends XComponent {
	constructor(props) {
		super(props);
	...
	getReportError(error) {
		FuncUtil.logDevice('error', `FINANCIAL REPORT ERROR: ${error}`);
		this.setState({
			error,
			isLoading: false
		});
	...
	getReport() {
		try {
			...
			const type = ENUM.REPORT_FROM_FILE_TYPE.FINANCIAL_TRANSACTIONS;
			...
			// url = `${url}?from=${fromStartDayUTC}&to=${toStartDayUTC}`
			...
			Http.get({
				...
				.then((res) => {
					try {
						FuncUtil.logDevice(
							'info',
							`FINANCIAL WRITEFILE PATH: ${this.dic.path}`
						);
						RNFetchBlob.fs
							.writeFile(this.dic.path, res, 'base64')
							.then(() => {
								FuncUtil.logDevice('info', `FINANCIAL SUCCESS`);
								this.setState(
									{
										error: '',
	...
	setDuration(duration) {
		...
		this.view && this.view.fadeOut(350).then();
	...
	checkFirstLastDuration(index) {
		const listDurationLength = this.dic.listDuration.length;
		if (index === 0) {
			return this.dic.position.FIRST;
		} else if (index === listDurationLength - 1) {
			return this.dic.position.LAST;
		} else {
			return this.dic.position.MID;
		}
	}
			...
			>
				{/* <Pdf
                    ...
                    onLoadComplete={numberOfPages => {
                        FuncUtil.logDevice('info', `onLoadComplete`)
                        console.log('numberOfPages', numberOfPages)
                        FuncUtil.logDevice('info', `onLoadComplete-numberOfPages ${numberOfPages}`)
                    }}
                    onPageChanged={currentPage => {
                        FuncUtil.logDevice('info', `onPageChanged`)
                        console.log('currentPage', currentPage)
                        FuncUtil.logDevice('info', `onLoadComplete-currentPage ${currentPage}`)
                    }}
                    onError={error => {
                        this.getReportError(error)
                        FuncUtil.logDevice('info', `onLoadComplete-error ${error}`)
                    }}
                /> */}
			</TransitionView>
		);
			...
			<FallHeader
				...
				header={
					<Header
						...
						title={I18n.t('financialTransactions')}
