		...
		return (
			<View style={styles.headerBorderTop}>
				<View style={{ flexDirection: 'row' }}>
					<Text style={[styles.col50, CommonStyle.textMainHeader]}>
					...
					>
						{`${this.getText('buyUpper')} / ${this.getText('sellUpper')}`}
					</Text>
					<Text
						style={[
							styles.col25,
				...
				<View style={{ flexDirection: 'row' }}>
					<Text style={[styles.col50, CommonStyle.textSubHeader]}>
						{
							this.getText('securityUpper')
						}
					</Text>
					<Text style={[styles.col25, CommonStyle.textSubHeader]} />
				</View>
			</View>
		);
	}
...
class SearchPortfolio extends React.Component {
	...
	render() {
		...
		// push openprice
		result.push(<OpenPriceHeader handleShowPortfolioDetail={this.props.handleShowPortfolioDetail} isCollapsed={this.state.isCollapsed} onPress={this.onPress} key='searchPortfolioPriceHeader' />);

		if (!this.state.isCollapsed) {
			result.push(<View style={{ height: 1 * 24 }} key='searchPortfolioFooter' />);
			return result;
		}
		result.push(<ListTransaction handleCloseExpand={this.handleCloseExpand} handleShowPortfolioDetail={this.props.handleShowPortfolioDetail} symbol={this.props.symbol} key='searchPortfolioTransaction' />);
