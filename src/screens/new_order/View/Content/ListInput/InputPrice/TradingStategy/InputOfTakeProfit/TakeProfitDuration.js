...
import InputDate from '~/screens/new_order/View/Content/ListInput/InputPrice/TradingStategy/InputOfTakeProfit/TakeProfitInputDate.js'

import { connect, useSelector, shallowEqual, useDispatch } from 'react-redux'
import { changeDurationTP, changeDatePeriod } from '~/screens/new_order/Redux/actions.js'
...
const selectState = createSelector(
    ...
    newOrder => ({
        ...
        duration: newOrder.takeProfitLoss.duration,
        layout: newOrder.layout
    })
)
const TakeProfitDuration = React.memo((props) => {
    const { disabled } = props
    const { isLoadingOrderAttribute, duration, layout } = useSelector(selectState, shallowEqual)
    const dispatch = useDispatch()
    const changeDuration = useCallback((params) => {
        dispatch(changeDurationTP(params))
    }, [])
...
export default TakeProfitDuration
