...
import { changeLimitPriceTP, changeFocusInput } from '~/screens/new_order/Redux/actions.js'
...
function useOnListenError({ updateError }) {
    ...
    return useEffect(() => {
        const id = Emitter.addListener(channel, null, ({ msg, type, key }) => {
            key === TYPE_ERROR_ORDER.TAKE_PROFIT_LIMIT_PRICE_ERROR && updateError && updateError({
...
const selectState = createSelector(
    ...
    newOrder => ({
        ...
        limitPrice: newOrder.takeProfitLoss.limitPrice
    })
)
const TakeProfitLimitPrice = React.memo(({ disabled }) => {
    ...
    const onFocus = useCallback((ref) => {
        ...
        dispatch(changeFocusInput(NEW_ORDER_INPUT_KEY.TAKE_PROFIT_LIMIT_PRICE))
        InputModel.setInputFocus(NEW_ORDER_INPUT_KEY.TAKE_PROFIT_LIMIT_PRICE)
    }, [limitPrice])
    ...
    const onChangeText = useCallback((text) => {
        ...
        const refViewWrapper = refTextInput.current && refTextInput.current.getRef()
        refViewWrapper.setNativeProps && refViewWrapper.setNativeProps({ style: { borderWidth: 1, borderColor: CommonStyle.fontBorder } })
        dispatch(changeLimitPriceTP(textValidate))
    }, [])
...
TakeProfitLimitPrice.propTypes = {}
TakeProfitLimitPrice.defaultProps = {}
export default TakeProfitLimitPrice
