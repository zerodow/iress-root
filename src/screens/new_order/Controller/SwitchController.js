...
import { initStore } from '~/screens/new_order/Redux/actions.js'
import Enum from '~/enum'
import CommonStyle from '~/theme/theme_controller'
import { setDisableTabBuySell, setTabTrading } from '~/screens/new_order/Model/TabModel.js'
import { setType, setOrderDetail } from '~/screens/new_order/Model/OrderEntryModel.js'
...
const getExpiryTime = ({ expiryTime }) => {
    return expiryTime ? getExpiryTimeUTC({ expiryTime }) : (new Date()).getTime() + 1000 * 60 * 60 * 24
}
export function getObjectNewOrderReduxFromOrderDetail({ data }) {
    ...
    expiryTime = expiryTime ? getExpiryTimeUTC({ expiryTime }) : (new Date()).getTime() + 1000 * 60 * 60 * 24
    let {
    ...
    orderType === OrderTypeEnum.LIMIT && limitPrice !== null && setLimitPrice(limitPrice)
