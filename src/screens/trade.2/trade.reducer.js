...
export const setShowedInfo = (state, { info }) =>
	state.merge({ childShowedInfo: info });

export const setExpandIndex = (state, { i }) => {
	if (i === state.expandIndex) {
		return state.merge({ expandIndex: -1 });
	} else {
		return state.merge({ expandIndex: i });
	}
};
...
export const reducer = createReducer(INITIAL_STATE, {
	...
	[Types.SET_SHOWED_INFO]: setShowedInfo,
	[Types.SET_EXPAND_INDEX]: setExpandIndex
});
