...
import { ActivityIndicator, AppState, Keyboard, Text, TextInput, TouchableOpacity, View, Dimensions, Platform } from 'react-native'
...
const marginTopButtons = Platform.OS === 'ios' ? (isIphoneXorAbove() ? 32 : 20) : (height > 2000 ? 32 : 20);
...
export class SendCode extends XComponent {
	...
	renderSend() {
		...
		return <TouchableOpacity
			...
			<Text style={[homePageDescriptionText, disableStyle, { fontSize: CommonStyle.fontSizeXS }]}>
				{I18n.t('resend')}
			</Text>
		</TouchableOpacity>
	}
...
export class ContentCompleteSignup extends XComponent {
	constructor(props) {
		super(props);
		...
		this.dic.timeoutExpired = setTimeout(() => {
			this.setState({
				...
				const obj = {
					...
					content: I18n.t('completeSignUpExpire')
				};
				Emitter.emit(channelChangeHeader, obj)
			})
		}, timeout);
	...
	requestForgotSuccess() {
		...
		const obj = {
			title: I18n.t('enterNewPassword'),
			content: I18n.t('completeSignUpNewPassword')
		};
		Emitter.emit(channelChangeHeader, obj);
		return true
	}
	...
	renderButtonCancel() {
		...
		>
			<Text
				style={[
					homePageDescriptionText,
					{ color: CommonStyle.fontWhite, opacity: this.state.cancelDisable ? 0.78 : 1 }
				]}
			>
				{I18n.t('cancel')}
			</Text>
	...
	renderUserNameInput() {
		...
		return <View style={{ flexDirection: 'row' }}>
			<View style={{ flexDirection: 'row', height: 48, flex: 1, alignItems: 'flex-end', marginRight: 8 }}>
				...
				>
					<Icon
						style={[{ opacity: 1, color: CommonStyle.fontWhite }]}
						name={'md-close'}
						size={16}
					/>
				</TouchableOpacity>
				...
				<View style={{ flexDirection: 'row' }}>
					<TextInput
						...
						style={[
							...
							{
								color: CommonStyle.fontWhite,
								borderBottomColor: this.state.passwordMatched ? 'gray' : 'red'
							}
						]}
					/>
					<TouchableOpacity
						style={[rightIcon, { borderBottomColor: this.state.passwordMatched ? 'gray' : 'red' }]}
						activeOpacity={1}
	...
	renderForgotPassword() {
		return (
			<View>
				<View style={{ marginHorizontal: 48, marginTop: marginTopButtons }}>
					{
				...
				<View style={{ paddingTop: marginTopButtons }}>
					{
...
export default connect(mapStateToProps, mapDispatchToProps)(ContentCompleteSignup);
