...
import RowNewsLoading from '~/screens/news_v3/view/list_news_wrapper/wrapper_content/components/FlatListSequenceAnimation/RowNewsItemLoading.js'
...
export default class FlatListAni extends Component {
    ...
    shouldComponentUpdate(nextProps) {
        return this.props.contentContainerStyle.paddingTop !== nextProps.contentContainerStyle.paddingTop
    }
    runAnimation = () => {
    ...
    renderItem = ({ index, item }) => {
        return (
            <SquenceView key={`SquenceView-${index}-${item.news_id}`} progressValue={this.progressValue} duration={this.duration} type={this.state.type} key={`${index}-${item}`} index={index} totalCountData={this.props.data.length}>
                ...
                <RowNewsLoading item={item} />
            </SquenceView>
        )
    }

    render() {
        console.info('DCM contentContainerSttyle', this.props.contentContainerStyle)
        return (
            <FlatList
