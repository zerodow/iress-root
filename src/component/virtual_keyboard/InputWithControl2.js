...
const InputWithControl = React.forwardRef(({ formatValueWhenBlur, formatValueWhenFocus, styleWrapper, styleText, propsTextInput = {}, formatValue, step = 1, decimal = 0, alias = '', allowNegative, ...rest }, ref) => {
    ...
    const onFocus = useCallback(() => {
        ...
        changeText(formatValueWhenFocus(text))
        dic.current.text = formatValueWhenFocus(text)
    }, [text])
    const onBlur = useCallback(() => {
        changeText(formatValueWhenBlur(text))
        dic.current.text = formatValueWhenBlur(text)
    ...
    const onReduction = useCallback(() => {
        let newText = isNaN(parseFloat(parseFloat(dic.current.text).toFixed(decimal))) ? 0 : parseFloat(parseFloat(dic.current.text).toFixed(decimal))
    ...
    return (
        <View
            ...
            <IncreaseButton limitNumberLength={16} text={text} onPress={onIncrease} />
        </View>
    )
})
