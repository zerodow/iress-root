...
import { StyleSheet, Text, View, processColor, Platform, Dimensions } from 'react-native';
...
class LineChartScreen extends React.Component {
	...
	componentDidMount() {
		this._lineChart && this._lineChart.getOffset()
	}
	...
	handleSelect(event) {
		...
		if (this.isLongPress) {
			if (entry.data && entry.data.marker) {
				this.dicPoint.push({ ...entry, touchPos: { pageX: this.pageX } });
			...
			if (Platform.OS === 'android') this.isLongPress = false
		} else {
			this.dicPoint = [];
			this.highlights([]);
	...
	renderInfo() {
		...
		if (secondPoint) {
			...
			let leftFirstPoint = firstPoint.touchPos.pageX
			let leftSecondPoint = firstPoint.touchPos.pageX <= secondPoint.touchPos.pageX
		...
		if (firstPoint && secondPoint) {
			...
			// 1> Gan trai
			if (firstPoint.touchPos.pageX - min < textWidth || secondPoint.touchPos.pageX - min < textWidth) {
			...
			// 2> Gan phai
			if (max - firstPoint.touchPos.pageX < textWidth || max - secondPoint.touchPos.pageX < textWidth) {
				if (firstPoint.touchPos.pageX <= secondPoint.touchPos.pageX) {
					// First <= Second -> Second lam chuan
