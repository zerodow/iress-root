...
export default class BoxShadow extends Component {
  render = () => {
    const { heightShadow } = this.props
    // get the shadow settings and give them default values
    const { width = WIDTH_DEVICE, height = 0, color = CommonStyle.color.shadow, border = 5, radius = 20, opacity = 0.5, x = 0, y = 0, style = {
      zIndex: 9,
      position: 'absolute',
      backgroundColor: CommonStyle.backgroundColor,
      top: 0,
      left: 0,
      right: 0
    } } = this.props.setting || shadowOpt

    // define the lengths
    const lineWidth = border;
    const rectWidth = width - radius * 2;
    const rectHeight = height > (radius * 2) ? height - radius * 2 : height
    ...
    return (
      <View style={[{ position: 'relative', width: width, height: height }, style]}>
        <Svg height={heightShadow || height + lineWidth * 2 + radius * 2} width={width + lineWidth * 2 + radius * 2} style={{ position: 'absolute', top: y - lineWidth, left: x - lineWidth }}>
          <Defs>
