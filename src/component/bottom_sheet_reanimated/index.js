...
import { Dimensions, Platform, View, KeyboardAvoidingView } from 'react-native';
...
export default class BottomSheetBehavior extends Component {
	...
	constructor(props) {
		...
		const destinationPointRunContent = add(
			masterOffseted,
			multiply(tossForMaster, this.velocity)
		);
		...
		const currentSnapPointRunContent = (i = 0) =>
			i + 1 === snapPoints.length
				? snapPoints[i]
				: cond(
						lessThan(
							destinationPointRunContent,
							middlesOfSnapPoints[i]
						),
						snapPoints[i],
						currentSnapPoint(i + 1)
				  );
			...
			cond(
				...
				[
					cond(eq(this.panState, State.ACTIVE), [
						...
						set(
							translationMaster,
							sub(translationMaster, diffPres)
						),
						cond(
							greaterThan(translationMaster, 10),
							set(isMasterRun, 1)
						)
					]),
	...
	static getDerivedStateFromProps(props, state) {
		...
		if (state && state.snapPoints) {
			state.snapPoints.forEach((s, i) =>
				s.setValue(
					sortedPropsSnapPints[0].val - sortedPropsSnapPints[i].val
				)
			);
