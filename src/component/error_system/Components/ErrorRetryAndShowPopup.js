...
const useHandleReTry = ({ onReTry, onShowPopUp }) => {
    ...
    const handleReTry = useCallback(() => {
        ...
        if (isLoading) return
        if (dic.current.numRetry > 2) {
            dic.current.timoutSetFail = setTimeout(() => {
                onShowPopUp()
                dic.current.numRetry = 0
            }, 2000);
        } else {
            onReTry && onReTry()
        }
...
const ErrorRetryAndShowPopup = ({ code, onReTry }) => {
...
ErrorRetryAndShowPopup.propTypes = {}
ErrorRetryAndShowPopup.defaultProps = {}
const styles = StyleSheet.create({})
export default ErrorRetryAndShowPopup
